...
//  Serial.begin(115200);
  ...
  for (int i = 0; i < 40; i++) { mass_36[i] = 0; } // чистим массив поиска
}
//-------------------------------------------------------------
void loop() 
{
  ...
  if (poisk == 1) 
  {
    if (millis() - gr_milis >= scrin_GRAF * 1000) //счет для графика
	{
	  ...
	  for (uint8_t s = 40; s >= (41 - scrin_GRAF); s--)
	   {
	     shet_gr = shet_gr + mass_36[s];
//Serial.print(mass36[s]);
//Serial.print(" ");
	   }
...
//Serial.println(GRAPH_count);	
//Serial.println (mass_p[82]);      
    }
    if (timer_seconds != count_and_dose_seconds) {
      count_and_dose_seconds = timer_seconds;
